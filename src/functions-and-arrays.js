...
function findLongestWord(arr) {
  if (arr.length === 0) {
    ...
    for (let maxLengthWord of arr) {
      if (maxLengthWord.length > palabraMasLarga.length) {
        palabraMasLarga = maxLengthWord;
      }
    }
...
function averageNumbers(avg) {
  ...
  return mediaNumbers / avg.length;
}
...
function averageWordLength(arrValue) {
...
function uniquifyArray(arrValue) {
  let variableFiltrada = [];
  if (arrValue.length === 0) {
    return null;
  } else {
    for (let deleteDuplicated of arrValue) {
      if (variableFiltrada.includes(deleteDuplicated)) {
        continue;
      } else {
        variableFiltrada.push(deleteDuplicated);
      }
    }
...
function howManyTimes(arr, palabra) {
  let counter = 0;
  for (let repeticiones of arr) {
    if (palabra === repeticiones) {
      counter++;
    }
  }
  return counter;
